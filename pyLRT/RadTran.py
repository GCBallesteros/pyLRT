...
class RadTran:
    ...
    def run(
        ...
        **parse_kwargs,
        ...
        inputstr = "\n".join(
            ["{} {}".format(name, self.options[name]) for name in self.options.keys()]
        )
        if print_input:
            print(inputstr)
            print("")
        ...
        if parse or (
            parse is None and (self.parser is not None or "parser" in parse_kwargs)
        ):
    ...
    def _cloud_input(self, type="liquid", print_input=False):
        ...
        assert self.ice_cloud is not None

        if type == "liquid":
            cloudstr = format_cloudstr(
                self.cloud["z"], self.cloud["lwc"], self.cloud["re"]
            )
            ...
            cloudstr = format_cloudstr(
                self.ice_cloud["z"], self.ice_cloud["iwc"], self.ice_cloud["re"]
            )
            tmpfile.write(cloudstr.encode("ascii"))
            tmpfile.close()
...
def _read_table(f, start_idstr, labels, wavelengths, regrid=False):
    ...
    if regrid:
        ...
        optprop = xr.Dataset(
            ...
            coords={"lc": range(optprop.shape[1]), "wvl": wavelengths["wvl"]},
        )
    return optprop
...
def _read_verbose(f, regrid=False):
    ...
    proflabels = ["lc", "z", "p", "T", "air", "o3", "o2", "h2o", "co2", "no2", "o4"]
    profiles = xr.Dataset(
        {
    ...
    redist = _read_table(
        f, "*** setup_redistribute", redistlabels, wavelengths, regrid=regrid
    )
    optprop = _read_table(
        f, "*** optical_properties", optproplabels, wavelengths, regrid=regrid
    )
...
def _map_to_outputwvl(data, wavelengths):
    ...
    for i in range(wavelengths["wvl"].shape[0]):
        opdata[np.where(output_wvl == wavelengths["OutputWVL"].data[i])[0]] += (
            wavelengths["Weights"].data[i] * data[i]
        )
    return opdata
